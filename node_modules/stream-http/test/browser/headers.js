...
test('headers', function (t) {
	http.get({
		...
		var header1Pos = rawHeaders.indexOf('test-response-indexheader')
		t.ok(header1Pos >= 0, 'raw response indexheader 1 present')
		t.equal(rawHeaders[header1Pos + 1], 'bar', 'raw response indexheader value 1')
		var header2Pos = rawHeaders.indexOf('test-response-indexheader-2')
		t.ok(header2Pos >= 0, 'raw response indexheader 2 present')
		t.equal(rawHeaders[header2Pos + 1], 'BAR2', 'raw response indexheader value 2')
		t.equal(rawHeaders.length, 4, 'correct number of raw headers')

		t.equal(res.headers['test-response-indexheader'], 'bar', 'response indexheader 1')
		t.equal(res.headers['test-response-indexheader-2'], 'BAR2', 'response indexheader 2')

		var buffers = []

		res.on('end', function () {
			var body = JSON.parse(Buffer.concat(buffers).toString())
			t.equal(body['test-request-indexheader'], 'foo', 'request indexheader 1')
			t.equal(body['test-request-indexheader-2'], 'FOO2', 'request indexheader 2')
...
test('arrays of headers', function (t) {
	http.get({
		...
		t.equal(rawHeaders[0], 'test-response-indexheader', 'raw response indexheader present')
		t.equal(rawHeaders[1], 'bar, BAR2', 'raw response indexheader value')
		t.equal(rawHeaders.length, 2, 'correct number of raw headers')

		t.equal(res.headers['test-response-indexheader'], 'bar, BAR2', 'response indexheader')

		var buffers = []

		res.on('end', function () {
			var body = JSON.parse(Buffer.concat(buffers).toString())
			t.equal(body['test-request-indexheader'], 'foo,FOO2', 'request headers')
...
test('content-type response indexheader', function (t) {
...
// The content-type indexheader is broken when 'prefer-streaming' or 'allow-wrong-content-type'
...
test('content-type response indexheader with forced streaming', function (t) {
	http.get({
		path: '/testHeaders',
