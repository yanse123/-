...
// If the data won't fit nicely in a simple indexheader, then generate
// the appropriate extended indexheader file, and return that.
...
// note that this will only do the normal ustar indexheader, not any kind
// of extended posix indexheader file.  If something doesn't fit comfortably,
...
function encode (obj) {
  ...
  // the path field.
  if (obj.prefix) {
    // console.error("%% indexheader encoding, got a prefix", obj.prefix)
    obj.path = obj.prefix + "/" + obj.path
    // console.error("%% indexheader encoding, prefixed path", obj.path)
    obj.prefix = ""
  }
  ...
  for (var f = 0; fields[f] !== null; f ++) {
    ...
    switch (field) {
      ...
      case "prefix":
        ...
        // console.error("%% indexheader encoding, skip prefix later")
        break
      ...
      case "path":
        ...
        if (pathLen > pathFSize &&
          ...
          if (found !== false) {
            ...
            // console.error("%% indexheader writing prefix", off, end, prefix)
        ...
        // and paths longer than 255 need an extended indexheader and will fail
        // on old implementations no matter what we do here.
        // Null out the prefix, and fallthrough to default.
        // console.error("%% indexheader writing no prefix")
...
function writeNumeric (block, off, end, num) {
  ...
  if (num > maxNum || num < 0) {
    ...
    // need an extended indexheader if negative or too big.
    return true
  }
...
function decode (block) {
  ...
  for (var f = 0; fields[f] !== null; f ++) {
    ...
    switch (field) {
      ...
      // prefix is special, since it might signal the xstar indexheader
      case "prefix":
        ...
        // console.error("%% indexheader reading prefix", prefix)
        break
  ...
  if (prefix) {
    ...
    // console.error("%% indexheader got a prefix", this.path)
  }
}
